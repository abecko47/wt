...
var articlesPerPage=articlesPagination;
...
function navHtml(startIndex, articlesCount, articlesTotalCount){
    ...
    var htmlKod="";
    if(articlesCount>0){
        htmlKod+=" <button style='margin-bottom: 5px; margin-top: 5px;' class=\"blog-button\" onclick=\"previousArticles("+articlesCount+");\">"+"Previous</button>";
        htmlKod+=" <button style='margin-bottom: 5px; margin-top: 5px;' class=\"blog-button\" onclick=\"nextArticles("+articlesCount+");\">"+"Next</button>";
        htmlKod+="<hr> ";
        htmlKod+="(Displaying articles  "+(startIndex+1)+" to "+(startIndex+articlesCount)+" from "+ articlesTotalCount  +" articles.) <br /> <br />";

        //pagination
        pages = Math.floor(articlesTotalCount / articlesPerPage);
        if(articlesTotalCount % articlesPerPage !== 0) pages++;
        current_page = Math.round((startIndex+articlesCount)/articlesCount);
        if(current_page > pages) current_page = pages;
        htmlKod += "<div id='pages-list'>";
        if(current_page < 2) {
            for (i = current_page; i < 3+current_page; i++) {
                htmlKod += "<div onclick='goToPage("+i+", "+articlesCount+");' class='page-link' id='page-"+i+"'>"+i+"</div>";
            }
            htmlKod += "<div class='page-link'>...</div>";

            htmlKod += "<div onclick='goToPage("+pages+", "+articlesCount+");' class='page-link' id='page-"+pages+"'>"+pages+"</div>";
            htmlKod += "</div>";
        } else  if((pages-2 > current_page)){
            htmlKod += "<div onclick='goToPage("+1+", "+articlesCount+");' class='page-link' id='page-"+1+"'>"+1+"</div>";
            htmlKod += "<div class='page-link'>...</div>";

            for (i = current_page; i < 3+current_page; i++) {
                htmlKod += "<div onclick='goToPage("+i+", "+articlesCount+");' class='page-link' id='page-"+i+"'>"+i+"</div>";
            }
            if(pages-3 !== current_page) {
                htmlKod += "<div class='page-link'>...</div>";
                htmlKod += "<div onclick='goToPage("+pages+", "+articlesCount+");' class='page-link' id='page-"+pages+"'>"+pages+"</div>";
            }
            htmlKod += "</div>";
            ...
            for (i = current_page; i < pages+1; i++) {
                htmlKod += "<div onclick='goToPage("+i+", "+articlesCount+");' class='page-link' id='page-"+i+"'>"+i+"</div>";
            }
            else {
                htmlKod += "<div onclick='goToPage("+pages+", "+articlesCount+");' class='page-link' id='page-"+pages+"'>"+pages+"</div>";
...
function goToPage(page, articlesCount) {
    offset = (page * articlesCount)-articlesCount;
    writeArticles2Html(offset, articlesPerPage, server, 'clanky', 'navigacia');
}

function nextArticles(articlesCount) {
    offset += articlesCount;
    writeArticles2Html(offset, articlesPerPage, server, 'clanky', 'navigacia');
}

function previousArticles(articlesCount) {
    offset -= articlesCount;
    writeArticles2Html(offset, articlesPerPage, server, 'clanky', 'navigacia');
}
